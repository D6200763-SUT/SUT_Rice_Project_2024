...
def load_two_stage_wf(wf_root: Path, folds: list[int]) -> dict:
    ...
    # อ่าน spike metrics จาก cv_summary.csv (ถ้ามี)
    ...
    for fold in folds:
        ...
        # Spike-window จาก cv_summary.csv
        if df_cv is not None:
            row = df_cv[df_cv["fold"] == fold]
            if not row.empty:
                for col, lst in [("spike_rmse_raw", spike_rmses),
                                  ("spike_mae_raw",  spike_maes)]:
                    if col in row.columns:
                        lst.append(float(row[col].values[0]))
...
def write_ablation_md(single: dict, two: dict, out_dir: Path) -> Path:
    ...
    lines.append(
        f"| Test periods | {s['test_period']} | {t['test_period']} | 3x coverage |"
    )
    ...
    lines += [
        ...
        f"| AUC-ROC | — | {fmt(t['auc_s1'])} | New capability |",
    ]
    ...
    lines += [
        ...
        f"| R2 (log1p) | {fmt(s['r2_log1p'])} | {fmt_ms(t['r2_log1p'], t['r2_log1p_sd'])} | {delta_r2:+.3f} |",
        f"| MAE (log1p) | {fmt(s['mae_log1p'])} | {fmt(t['mae_log1p'])} | — |",
        f"| RMSE raw (BPH/tiller) | {fmt(s['rmse_raw'], 1)} | {fmt_ms(t['rmse_raw'], t.get('rmse_raw_sd', float('nan')), 1)} | {delta_rmse:+.1f} |",
    ]

    lines += [
        "| **Spike-window Metrics** | | | |",
        f"| MAE raw (spike rows) | — | {fmt(t['spike_mae_raw'], 1)} BPH/tiller | New metric |",
        f"| RMSE raw (spike rows) | — | {fmt(t['spike_rmse_raw'], 1)} BPH/tiller | New metric |",
    ]

    # Apples-to-apples: Fold 4 only
    f4_delta_r2   = t["fold4_r2_log1p"] - s["r2_log1p"]
    f4_delta_rmse = t["fold4_rmse_raw"] - s["rmse_raw"]
    lines += [
        "",
        ...
        "|--------|-------------|-----------------|-------|",
        f"| R2 (log1p) | {fmt(s['r2_log1p'])} | {fmt(t['fold4_r2_log1p'])} | {f4_delta_r2:+.3f} |",
        f"| RMSE raw (BPH/tiller) | {fmt(s['rmse_raw'], 1)} | {fmt(t['fold4_rmse_raw'], 1)} | {f4_delta_rmse:+.1f} |",
        "",
...
def plot_ablation_bars(single: dict, two: dict, wf_root: Path, out_dir: Path) -> None:
    ...
    ax.set_xticks([0, 1])
    ax.set_xticklabels(["Single-stage\n(Fold 4)", "Two-stage\n(3 folds mean)"], fontsize=8)
    ax.set_ylabel("R2 (log1p)", fontsize=9)
    ax.set_title("R2 (log1p)", fontsize=9)
    ax.set_ylim(0, 0.65)
    ax.axhline(0.484, color="#E24B4A", linestyle="--", linewidth=1.2, label="Baseline 0.484")
    ...
    bars = ax.bar([0, 1], vals, width=bar_w,
                  color=[colors["single"], colors["two"]])
    ax.set_xticks([0, 1])
    ax.set_xticklabels(["Single-stage\n(Fold 4)", "Two-stage\n(Fold 4 only)"], fontsize=8)
    ax.set_ylabel("RMSE (raw, BPH/tiller)", fontsize=9)
    ax.set_title("RMSE raw -- Fold 4 only\n(apples-to-apples)", fontsize=9)
    ax.set_ylim(0, max(v for v in vals if not math.isnan(v)) * 1.3)
    for bar, v in zip(bars, vals):
        if not math.isnan(v):
            ax.text(bar.get_x() + bar.get_width() / 2,
    ...
    if not math.isnan(vals[0]) and not math.isnan(vals[1]):
        ...
        ax.annotate("", xy=(1, vals[1] + 8), xytext=(0, vals[0] - 8),
                    arrowprops=dict(arrowstyle="->", color="#639922", lw=1.5))
        ax.text(0.5, mid_y, f"{delta:+.0f}", ha="center", fontsize=8, color="#639922")

    # --- Plot 3: Stage 1 Recall per fold ---
    ax = axes[2]
    fold_recalls = []
    fold_labels  = []
    fold_years   = {2: "2017", 3: "2018", 4: "2019"}
    for fold in [2, 3, 4]:
        p = wf_root / f"fold{fold}" / "stage1" / "metrics_stage1.json"
        if p.exists():
            s1 = load_json(p)
            fold_recalls.append(s1.get("recall", float("nan")))
            fold_labels.append(f"Fold {fold}\n(test {fold_years[fold]})")
    ...
    if fold_recalls:
        ...
        valid = [r for r in fold_recalls if not math.isnan(r)]
        if valid:
            ax.axhline(np.mean(valid), color="#639922", linestyle=":",
                       linewidth=1.5, label=f"Mean {np.mean(valid):.3f}")
        ...
        ax.text(0.5, 0.5, "No Stage 1 data", ha="center",
...
def plot_spike_detection(single_dir: Path, wf_fold4_dir: Path, out_dir: Path) -> None:
    ...
    if "station_id" in df_t.columns:
        df_t_st = df_t[df_t["station_id"] == best_station].copy()
    else:
        df_t_st = df_t.copy()
    ...
    ax.set_ylabel("BPH (tiller)", fontsize=9)
    ax.set_title("Single-stage CNN-LSTM", fontsize=9)
    ax.legend(fontsize=8, loc="upper right")

    # ล่าง: true vs two-stage + stage1 alerts
    ax = axes[1]
        ...
        if stage1_pred_path.exists():
            ...
            if "date" in df_s1_st.columns and "y_pred_spike" in df_s1_st.columns:
                ...
                merged = df_t_st.merge(
                    df_s1_st[["date", "y_pred_spike"]], on="date", how="left"
                )
                ...
                ax.legend(
                    handles=[
                        ...
                        plt.Line2D([0], [0], color="#185FA5", lw=1, ls="--",
                                   label="Two-stage pred"),
                        mpatches.Patch(facecolor="#E24B4A", alpha=0.2,
                                       label="Stage 1 alert"),
                    ],
...
def parse_args(argv=None):
    ...
    p.add_argument("--out_dir", default="results/ablation_study")
...
def main(argv=None):
    ...
    print(f"  R2_log1p={single['r2_log1p']:.3f}  RMSE_raw={single['rmse_raw']:.1f}")

    print("\n=== Loading two-stage walk-forward results ===")
    two = load_two_stage_wf(wf_root, folds)
    print(f"  R2_log1p={two['r2_log1p']:.3f} +/- {two['r2_log1p_sd']:.3f}")
    ...
    plot_ablation_bars(single, two, wf_root, out_dir)
    ...
    print("-" * 67)
    for label, sk, tk in [
        ("R2 log1p",            "r2_log1p",      "r2_log1p"),
        ("RMSE_raw (all folds)", "rmse_raw",      "rmse_raw"),
        ("RMSE_raw (Fold 4)",    "rmse_raw",      "fold4_rmse_raw"),
        ...
        sv_str = "—" if (isinstance(sv, float) and math.isnan(sv)) else f"{sv:.3f}"
        tv_str = "—" if (isinstance(tv, float) and math.isnan(tv)) else f"{tv:.3f}"
        delta  = ("" if (isinstance(sv, float) and math.isnan(sv))
                      or (isinstance(tv, float) and math.isnan(tv))
                  else f"({tv - sv:+.3f})")
