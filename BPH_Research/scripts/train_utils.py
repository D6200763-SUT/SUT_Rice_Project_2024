...
# -*- coding: utf-8 -*-
"""
train_seq_utils_compat.py
Shared utilities for training models from sequences_window*_h*.npz
Compatible with older scikit-learn versions (no squared= in mean_squared_error).
"""
...
def _finite_counts(a: np.ndarray) -> Dict[str, int]:
    if not np.issubdtype(a.dtype, np.floating):
        return {"nan": 0, "inf": 0}
    return {"nan": int(np.isnan(a).sum()), "inf": int(np.isinf(a).sum())}


def assert_all_finite(name: str, arr: np.ndarray, max_examples: int = 5) -> None:
    a = np.asarray(arr)
    if not np.issubdtype(a.dtype, np.floating):
        return
    bad = np.isnan(a) | np.isinf(a)
    if bad.any():
        idx = np.argwhere(bad)[:max_examples].tolist()
        c = _finite_counts(a)
        raise ValueError(
            f"[NaN/Inf DETECTED] {name}: shape={list(a.shape)} dtype={a.dtype} "
            f"nan={c['nan']} inf={c['inf']} example_idx={idx}. "
            f"Fix upstream (feature engineering / scaling) then rebuild sequences."
        )


def compute_metrics(y_true: np.ndarray, y_pred: np.ndarray) -> Dict[str, float]:
    """MAE / RMSE / R2, compatible with old sklearn (no squared=)."""
    y_true = np.asarray(y_true).reshape(-1)
    y_pred = np.asarray(y_pred).reshape(-1)
    assert_all_finite("y_true", y_true)
    assert_all_finite("y_pred", y_pred)
...
def subset_by_station(
    ...
    return df[df["station_id"] == str(station_id)].sort_values("date")
