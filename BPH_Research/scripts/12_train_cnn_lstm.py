...
# -*- coding: utf-8 -*-
"""
12_train_cnn_lstm.py
Train a CNN-LSTM regression model on sequences_window*_h*.npz.
"""
...
def build_model(window: int, n_features: int, conv_filters: int, kernel_size: int, lstm_units: int, dropout: float, lr: float):
...
def parse_args(argv=None):
    p = argparse.ArgumentParser(description="Train CNN-LSTM on sequences npz")
...
def main(argv=None):
    ...
    model = build_model(window, n_features, args.conv_filters, args.kernel_size, args.lstm_units, args.dropout, args.lr)
