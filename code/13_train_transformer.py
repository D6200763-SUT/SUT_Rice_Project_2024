...
# -*- coding: utf-8 -*-
"""
13_train_transformer.py
Train a lightweight Transformer encoder regression model on sequences_window*_h*.npz.
"""
...
def build_model(window: int, n_features: int, d_model: int, num_layers: int, num_heads: int, ff_dim: int, dropout: float, lr: float):
...
def parse_args(argv=None):
    p = argparse.ArgumentParser(description="Train Transformer encoder on sequences npz")
...
def main(argv=None):
    ...
    model = build_model(window, n_features, args.d_model, args.num_layers, args.num_heads, args.ff_dim, args.dropout, args.lr)
